  ...
  try {
    ...
    if (config.is_active) {
      ...
      await startCopyEngine({
        ...
        categories: config.categories || [],
        followMode: config.follow_mode || 'all',
        minSharePrice: config.min_share_price != null ? parseFloat(config.min_share_price) : null,
        maxSharePrice: config.max_share_price != null ? parseFloat(config.max_share_price) : null,
        maxPositionSize: config.max_position_size != null ? parseFloat(config.max_position_size) : null,
        encryptedPrivateKey: wallet.encrypted_private_key,
        walletAddress: wallet.address,
      }, config.target_wallet);
  ...
  try {
    ...
    const traders = await Promise.all(configs.map(async c => {
      ...
      return {
        ...
        minSharePrice:   c.min_share_price != null ? parseFloat(c.min_share_price) : null,
        maxSharePrice:   c.max_share_price != null ? parseFloat(c.max_share_price) : null,
